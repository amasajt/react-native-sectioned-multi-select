  ...
  _filterItems = (searchTerm) => {
    ...
    items &&
      items.forEach((item) => {
        ...
        if (item[subKey]) {
          const newItem = Object.assign({}, item) // business unit
          newItem[subKey] = []
          item[subKey].forEach((sub) => {

            if (regex.test(this.getProp(sub, displayKey))) {
              newItem[subKey] = [...newItem[subKey], sub] // business unit + children (country)
            ...
            //////////////////
            // Temporary searching functionality for the additional level
            // TODO: To improve search algorithm
            /////////////////
            if(sub[subKey]) {
              const newItem2 = Object.assign({}, sub) // country
              newItem2[subKey] = []
              sub[subKey].forEach((sub2) => {

                if (regex.test(this.getProp(sub2, displayKey))) {

                  newItem2[subKey] = [...newItem2[subKey], sub2]  // country + children

                  newFilteredItems2 = this.rejectProp(
                    newFilteredItems,
                    (singleItem) => sub[uniqueKey] !== singleItem[uniqueKey]
                  )
                  newFilteredItems2.push(newItem2)
                  filteredItems = newFilteredItems2
                }
              })
            }
            //////////////////
      ...
      items.forEach((item) => {
        ...
        Array.isArray(item[subKey]) &&
          item[subKey].forEach((sub) => {
            ...
            Array.isArray(sub[subKey]) &&
              sub[subKey].forEach((sub2) => {
                newItems = [...newItems, sub2[uniqueKey]]
              })
          })
      })
